...
GITHUB_REPO="kaytu-io/kaytu"
TMP_DIR=$(mktemp -d -t kaytu-install.XXXXXXXXXX)
...
get_release_version() {
    ...
    TMP_METADATA="${TMP_DIR}/kaytu.json"
...
download_hash() {
    HASH_URL="https://github.com/${GITHUB_REPO}/releases/download/v${VERSION}/kaytu_${VERSION}_checksums.txt"
    TMP_HASH="${TMP_DIR}/kaytu.hash"
    info "Downloading hash ${HASH_URL}"
    download "${TMP_HASH}" "${HASH_URL}"
    HASH_EXPECTED=$(grep " kaytu_${VERSION}_${OS}_${ARCH}.tar.gz$" "${TMP_HASH}")
...
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/v${VERSION}/kaytu_${VERSION}_${OS}_${ARCH}.tar.gz"
    info "Downloading binary ${BIN_URL}"
    TMP_BIN="${TMP_DIR}/kaytu.tar.gz"
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing kaytu to ${BIN_DIR}/kaytu"
    tar -xzof "${TMP_BIN}" -C "${TMP_DIR}"

    local CMD_MOVE="mv -f \"${TMP_DIR}/kaytu\" \"${BIN_DIR}\""
    if [ -w "${BIN_DIR}" ]; then
        eval "${CMD_MOVE}"
    else
        eval "sudo ${CMD_MOVE}"
